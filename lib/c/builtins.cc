...
int clause(solver s, atom* cl, int sz) {
  geas::vec<geas::clause_elt> elts;
  for(int ii : geas::irange(sz))
...
int all_different_int(solver s, intvar* xs, int sz) {
  ...
  */
  ...
  int dom = ub - lb + 1;
  if(p_xs.size() == dom) {
    geas::vec<int> srcs(p_xs.size(), 1);
    geas::vec<int> sinks(p_xs.size(), 1);

    geas::vec<geas::bflow> flows;
    for(int xi : geas::irange(p_xs.size())) {
      for(int k : geas::irange(dom)) {
...
int bipartite_flow(solver s, int* srcs, int srcs_sz, int* sinks, int sinks_sz, bp_flow* flows, int flows_sz) {
  geas::vec<int> src_vec;
  for(; srcs_sz; --srcs_sz, ++srcs)
    src_vec.push(*srcs);

  geas::vec<int> sink_vec; 
  for(; sinks_sz; --sinks_sz, ++sinks)
    sink_vec.push(*sinks);

  geas::vec<geas::bflow> flow_vec;
...
int cumulative(solver s, task* ts, int sz, int cap) {
  ...
  geas::vec<int> ds;
  geas::vec<int> rs;
  for(task t :geas::range(ts, ts+sz)) {
...
int cumulative_var(solver s, vtask* ts, int sz, intvar cap) {
  geas::vec<geas::intvar> xs;
  geas::vec<geas::intvar> ds;
  geas::vec<geas::intvar> rs;
  for(vtask t :geas::range(ts, ts+sz)) {
    xs.push(*get_intvar(t.start));
...
int cumulative_float(solver s, ftask* ts, int sz, float cap) {
  ...
  geas::vec<int> ds;
  geas::vec<float> rs;
  for(ftask t :geas::range(ts, ts+sz)) {
    xs.push(*get_intvar(t.start));
    ds.push(t.dur);
    rs.push(t.res);
  }
...
int disjunctive(solver s, dtask* ts, int sz) {
  geas::vec<geas::intvar> xs;
  geas::vec<int> ds;
  for(dtask t :geas::range(ts, ts+sz)) {
    xs.push(*get_intvar(t.start));
    ds.push(t.dur);
  }
...
table_id build_table(solver s, int arity, int* elts, int sz) {
  ...
  geas::vec< geas::vec<int> > rows;
  while(elts != end) {
    rows.push();
    geas::vec<int>& r(rows.last());
