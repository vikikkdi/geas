...
brancher new_bool_priority_brancher(var_choice varc,
  ...
  atom* end = vs + sz;
  geas::vec<geas::patom_t> sel;
  geas::vec<geas::brancher*> br;

  for(; vs != end; ++vs, ++bs) {
    sel.push(get_atom(*vs));
    br.push((geas::brancher*) (*bs));
  }
  return ((brancher) priority_brancher(get_varc(varc), sel, br));
}
...
brancher seq_brancher(brancher* bs, int sz) {
  geas::vec<geas::brancher*> branchers;
  brancher* end = bs + sz;
  for(; bs != end; ++bs)
...
brancher warmstart_brancher(atom* xs, int sz) {
  geas::vec<geas::patom_t> decs; 
  for(atom at : geas::range(xs, xs+sz)) {
